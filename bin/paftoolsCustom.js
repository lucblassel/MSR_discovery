...
function paf_mapeval(args)
{
	...
	function is_correct(s, b)
	{
		if (s[0] != b[0] || s[3] != b[3]) {
			if (s[0] != b[0]) {
				warn("Error in reference name")
			}
			if (s[3] != b[3]) {
				warn("Error in strand")
			}
			return false;
		}
		var o, l;
	...
	for (var q = max_mapq; q >= 0; --q) {
		...
		sum_tot2 += tot[q], sum_err2 += err[q];
		 print('Q', q, sum_tot, sum_err, (sum_err2/sum_tot2).toFixed(9), sum_tot2);
	}
