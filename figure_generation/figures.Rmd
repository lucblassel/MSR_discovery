...
library(knitr)
knitr::opts_chunk$set(message=FALSE, info=FALSE, warning=FALSE)
```
...
fmt_perc <- function(num) {
  ...
  if (num >= 0) {
    return(
      paste0("+", format(num, digits=0, width=3, justify="right"), "%")
    )
  }
  return (
    paste0(format(num, digits=0, width=3, justify="right"), "%")
...
p <- ggplot(data=background, aes(x=fracReads, y=cumErrorRate, group=renamed, colour=col)) + 
  ...
  labs(x="Fraction of reads mapped", y="Cumulative mapeval error rate") +
...
ggsave("./figures/figure5.svg", plot=p, height=3.5, width=10, units=c("in"))
p
```
...
p_ridges
```
The "A" entry is destined to be removed and is needed to scale everything correctly between facets. To obtain the final version of the figure the "A" track is removed using inkscape. The remaining facets are then moved closer to each other to avoid large gaps between facets. 
...
ggsave("./figures/figureE1.svg", plot=p, height=3.5, width=8, units=c("in"))
